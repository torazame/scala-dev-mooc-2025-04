...
(теорию вероятности), в виду того, что скалу очень часто применяют для анализа данных, то это будет полезно.
...
2. создать коллекцию обьектов этих классов, скажем 10000 элементов, и провести этот эксперимент (функция map)
...
object collections_1 {
  ...
  val res = urnList.map(extractBall)
